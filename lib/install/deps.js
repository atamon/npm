...
function isDep(tree, child) {
  ...
  return deps[child.package.name] != null || devDeps[child.package.name] != null
}

function resetMetadata(tree) {
  ...
  if (!tree.package._requiredBy) tree.package._requiredBy = []
  tree.package._requiredBy = tree.package._requiredBy.filter(function (req) {
    return req[0] === "#"
  })
  tree.requires = []
  ...
  tree.children.forEach(resetMetadata)
}
...
exports.loadRequestedDeps = function (args, tree, saveToDependencies, log, next) {
  ...
  }, andForEachChild(loadDeps, andMaybeLoadDevDeps(andFinishTracker(log, next))))
}

exports.removeDeps = function (args, tree, saveToDependencies, log, next) {
  validate('AOOF', [args, tree, log, next])
  asyncMap(args, function (name, done) {
    tree.removed  = tree.children.filter(function (child) { return child.package.name === name })
    tree.removed.forEach(function (child) {
      child.save = saveToDependencies
    })
